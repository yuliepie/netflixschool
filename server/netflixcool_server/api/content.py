...
netflix_contents_model = api.model(
    'netflix_contents', {
        ...
        'examples': fields.List(
                fields.Nested(sentences_model)
            )
    }
)

@api.route('/')
@api.param('content_id', 'The content idenifier')
@api.response(200, 'success')
@api.response(404, 'Content not found')
class HelloWorld(Resource):
    '''Netflix Contents 정보를 가져옵니다.'''
    @api.doc('GET Netflix Content')
    @api.marshal_with(netflix_contents_model)
    def get(self, content_id):

        netflix_content = NetflixContent.query.filter(NetflixContent.id==content_id).one()
